...
| **Scratchpad** | vault-scoped local draft; editable daily Markdown document after Save | saved document only | saved document only |
| **Whiteboard** | vault-scoped local draft; editable board JSON document after Save | saved document only | saved document only |
...
app is cleared. Use **Save as vault document** when you want to keep Scratchpad
content. Mesa saves it as the stable editable document
`Scratchpad/YYYY-MM-DD.md`, then reloads that document on the same day; it
syncs, appears in the file tree, and uses normal verified-write recovery.
Whiteboard saves its editable strokes as `Whiteboard/Board.mesa-board.json`.
Mesa reloads that board document, and an older local raster can remain as its
background during the first save. The board document syncs and is recoverable;
PNG export is no longer the persistence format.

Drafts belong to the vault and selected day. Refreshing the file list does not
reload a live draft. Delayed reads preserve newer typing; closing and reopening
during a save preserves subsequent edits. Closing Whiteboard during a stroke
retains the partial stroke. Undo stroke removes the last stroke. A save checks
the original disk baseline and vault session, so another editor or sync cannot
silently be overwritten. A conflicting save keeps the local draft and reports
the conflict.
Local draft records use `mesa:overlay-draft:*`, keyed by vault and document.
Existing `mesa:scratch:<date>` and `mesa:whiteboard` values are read as legacy
drafts when no scoped record exists; original legacy storage is retained.
