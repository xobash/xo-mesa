...
prefixes, alternate-data-stream syntax, reserved Windows device names, and
...
canonical parent before a verified create-only commit.
...
Missing nested folders are created one component at a time only after the
parent path passes confinement checks. A rejected path or oversized request
...
through Mesa's recovery UI. Older peers can still copy files, but do not
carry delete or rename intent until they upgrade.

Current manifests advertise journal support. File-only compatibility requires
an explicit missing/unsupported journal endpoint from a legacy peer that has
never exchanged a journal with this vault. A previously observed journal peer
remains journal-required even if it stops advertising the capability. Network,
authorization, invalid journal, and current-peer endpoint failures stop sync
...
unidentified older device still requires journal recovery; its acknowledgement
