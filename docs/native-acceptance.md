...
The process-tree values are partial native resource evidence. Renderer heap and
cache values explain which Mesa-owned surfaces are changing. Neither establishes
whole-app memory coverage by itself.

## Responsiveness and idle targets

These are Mesa product gates for the documented representative Mac and Windows
workload. Record the OS, hardware, workload, sample count, p95, and maximum;
do not treat a browser fixture or CI timing as native evidence.

| Measure | Target |
| --- | --- |
| Input to visible update during background work | p95 ≤ 100 ms |
| Foreground stall during background work | No reproducible stall > 500 ms |
| Warm ordinary note switch | p95 ≤ 200 ms |
| Warm search on the agreed corpus | p95 ≤ 300 ms |
| Repeated mixed-use retained memory after settling | ≤ 50 MiB unexplained increase per identical cycle |
| Idle/minimized app CPU | ≤ 1% of one logical core averaged over five minutes, excluding active sync/research |
...
establish a heap plateau. Treat those gaps as incomplete evidence and record
additional OS-level process coverage for a whole-app stability claim. The mixed
