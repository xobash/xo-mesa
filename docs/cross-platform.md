...
|---------|--------|-------|
| Windows | WebView2 111+ | Ships on Windows 10/11; the installer bootstraps or rejects an older runtime. |
...
Both packaged formats enforce `bundle.windows.minimumWebview2Version` at
`111.0.0.0`, matching the launcher's major-version floor. Use Tauri CLI 2.11+
for the shared NSIS/MSI option. Configuration and build checks do not replace
installation tests on Windows or signing verification of release artifacts.
