...
- the current document path, both vault-relative and absolute on disk
- up to 12 other open document paths, vault-relative only
- center/right pane layout

Mesa does not send the whole vault file list. Mesa sends the current document
path, not its contents. Pi can read that path when the user asks about the
document, so a large note does not consume model tokens before it is needed.
