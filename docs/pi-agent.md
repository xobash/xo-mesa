...
releasing. Focus that bar and press `Ctrl/Cmd+Shift+Enter` for the equivalent
keyboard transfer. The surface launches the actual `pi` CLI in a PTY, not a custom Mesa
chat interface.
...
The model-facing system context contains only what the user is directly accessing:

- vault name
- the current document name and vault-relative path
- up to 12 other open document paths, vault-relative only
- center/right pane layout

Mesa does not send the whole vault file list. Mesa sends the current document's
vault-relative path, not its absolute local path or contents. Pi starts in the
selected vault, so normal file operations remain relative to that working
directory. Absolute paths stay in local tool/process context and are not copied
...
window. All Pi surfaces render one shared `AgentSurface`
...
keyboard transfer, and named stateful controls — they cannot drift apart.
The workspace host keeps a usable minimum terminal width and height instead of
allowing Pi to collapse into a thin strip. Switching between the in-window
...
`Ctrl/Cmd+Shift+Enter` on the focused bar docks the same native surface back
without requiring pointer geometry. In-window floating headers also accept
Arrow movement and Shift+Arrow resize. Escape closes a floating Pi host in the
capture phase and is never forwarded to the terminal as the same keypress.
...
Deep Research can also be torn out from the Pi surface by dragging its own
header. That creates a decorated Mesa native research window which observes
the main renderer's single run. It is a monitoring surface, so it does not
create a second Pi session; the complete-run log export remains available
when the run reaches review or done.
See Tauri's
...
`Current Mesa document` block to that turn's system prompt. If the loopback
bridge is temporarily unavailable, the extension uses the bounded launch
context from `MESA_CONTEXT`. Detached
...
Decoded bytes do not go straight out. Each session runs two threads: one reads
the PTY as fast as it produces bytes and only *stages* what it decodes, and one
emits. Emitting per read meant a bulk Pi response — a long file dump, a big
tool result — became one `terminal://output` event per ~1 KB. A measured 1.3 MB
of output was 1,303 events in 0.02 s. On Windows every one of those is a JS
source string compiled on the app's UI thread plus a forced repaint, which is
enough to bury the message pump and freeze the window mid-answer.

So the emitter coalesces: bytes that arrive within 8 ms of the last send go out
together, capped at 64 KB per event. Output that arrives after a quiet moment —
your keystroke echo, a single line after a pause — is sent immediately and pays
no delay at all; only bytes arriving inside an already-open window wait, and 8
ms is half a frame. The reader never waits on the emitter, so Pi is never
slowed by its own output. Bytes and their order are preserved exactly, and any
...
- `MESA_CENTER_VIEW`
- `MESA_RIGHT_VIEWS`
- `MESA_CONTEXT` (bounded launch fallback)

The current document is the document currently selected in Mesa's editor or
preview. The per-turn context names this document explicitly. It does not
...
The focused Pi bar displays `Ctrl/Cmd+Shift+Enter` as the keyboard alternative.

Provider setup belongs inside the terminal workflow the user chooses to run.
Mesa does not maintain a separate provider panel, provider/model setting, or
API-key field for Pi. Provider credentials remain owned by the terminal tool's
own configuration rather than Mesa browser storage or troubleshooting exports.
On upgrade, Mesa removes obsolete provider, model, endpoint, and key fields
from its settings record while retaining unrelated future-version fields.
