...
export interface VaultFile {
  ...
  mtime?: number;
  /** File creation time (epoch ms), when available — used by graph timelapse. */
  createdAt?: number;
...
export interface NoteMeta {
  ...
  rawLinks: string[];
  /** Every local wiki/Markdown destination, including attachment embeds.
   * Used to select rename-repair candidates without opening unrelated notes. */
...
export interface Settings {
  ...
  peers: SyncPeer[];
  /** Removed device identities, applied lazily to each vault's journal. */
  retiredSyncPeers?: string[];
  /** Sidebar sort order. */
  sortMode: SortMode;
  ...
  typeFilter: string;
  /** Sidebar-only filter for saved web archives and sync conflict copies. */
  ...
  /** Obsidian-style graph display and force controls. Optional for old settings. */
  graphArrows?: boolean;
  graphTextFadeThreshold?: number;
  graphNodeSize?: number;
  graphLinkThickness?: number;
...
export interface GraphNode extends SimulationNodeDatum {
  ...
  ext: string;
  /** File timestamp used by the graph timelapse, when the vault provides it. */
  timelineTime?: number;
...
/** A named, reversible arrangement of Mesa's existing workspace surfaces. */
export type WorkspacePreset = "write" | "read" | "research";
