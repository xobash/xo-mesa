...
export function resolveBookmarkItems(
  bookmarks: readonly string[],
  files: readonly Pick<VaultFile, "relPath">[],
  ...
  for (const file of files) {
    ...
    fileSet.add(relPath);
    for (
      let slash = relPath.indexOf("/");
      slash >= 0;
      slash = relPath.indexOf("/", slash + 1)
    ) {
      folderSet.add(relPath.slice(0, slash));
    }
  }
  return bookmarks
    .map((rel) => {
      const isFile = fileSet.has(rel);
      const isFolder = !isFile && folderSet.has(rel);
...
export function BookmarksList() {
  ...
  const items = useMemo(() => {
    return resolveBookmarkItems(bookmarks, files, emptyFolders);
