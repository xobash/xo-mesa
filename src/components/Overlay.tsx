...
import { OVERLAY_BOTTOM, fitWin, keyboardWinPatch, mergeStoredWins, type OverlayWinRec } from "../lib/overlayWins";
import { readLocalNote, writeLocalNote } from "../lib/localNotes";
import { useOverlayDraft } from "./useOverlayDraft";
import { parseBoardDocument, type BoardDocument } from "../lib/overlayDraft";
import { SearchSurface } from "./SearchSurface";
import { TasksPanel } from "./TasksModal";
import { trapDialogFocus } from "./Modal";
import { parseTasks, type TaskItem } from "../lib/tasks";
import { claimKeyboardShortcut, isPlainShiftTab, isWindowTransferShortcut } from "../lib/shortcuts";
...
function trapOverlayFocus(e: ReactKeyboardEvent<HTMLElement>): void {
  if (isPlainShiftTab(e)) return;
  trapDialogFocus(e.currentTarget, e);
}
...
function fitToViewport(rec: WinRec, viewportHeight?: number): WinRec {
  if (typeof window === "undefined") return rec;
  return fitWin(rec, { width: window.innerWidth, height: viewportHeight ?? window.innerHeight });
}
const lsGet = (key: string): string => readLocalNote(key);
/** Layout/flag keys only. Losing an overlay window position or a "seen the
 *  tour" flag is invisible; losing what the user WROTE is not, so the two
 *  content surfaces below report their failures instead of using this. */
...
function FloatingWindow({
  ...
  return (
    ...
    >
      <div
        className="ov-win-bar"
        onPointerDown={startDrag}
        onKeyDown={handleTitleBarKeyDown}
        tabIndex={0}
        aria-label={`${title} window. Arrow keys move; Shift plus Arrow resizes.`}
        title="Arrow keys move · Shift+Arrow resizes"
      >
        <span className="ov-win-title">{title}</span>
        <span className="ov-window-hint" aria-hidden="true">Arrows move · Shift+arrows resize</span>
        {accessory}
        <button className="ov-win-close" onClick={onClose} aria-label={`Close ${title}`}>
          ×
        </button>
      </div>
...
/** Draft identity includes the vault session and the selected day. */
function Scratchpad() {
  const [day, setDay] = useState(localISO());
  const identity = useAppStore(s => `${s.vaultPath}\0${s.getVaultGeneration()}`);
  return <div className="ov-scratch-win">
    <input aria-label="Scratchpad date" type="date" className="text-input" value={day}
      onChange={e => setDay(e.target.value || localISO())} />
    <ScratchpadDocument key={`${identity}\0${day}`} day={day} />
  </div>;
}

export function ScratchpadDocument({ day }: { day: string }) {
  const draft = useOverlayDraft("scratchpad", `Scratchpad/${day}.md`, SCRATCH_PREFIX + day, day);
  return <>
    <textarea aria-label="Scratchpad draft" className="ov-scratch" placeholder="A quick note for this day…"
      value={draft.content} onChange={e => draft.edit(e.target.value)} />
    <button className="btn ghost ov-save-vault" disabled={!draft.ready || draft.saving || !draft.root}
      onClick={() => void draft.save()}>{draft.saving ? "Saving…" : "Save as vault document"}</button>
    {draft.status && <p className="ov-vault-status" role="status">{draft.status}</p>}
    {draft.error && <p className="ov-local-warn" role="alert">{draft.error}</p>}
  </>;
}

function Whiteboard() {
  const identity = useAppStore(s => `${s.vaultPath}\0${s.getVaultGeneration()}`);
  return <WhiteboardDocument key={identity} />;
}

export function WhiteboardDocument() {
  const draft = useOverlayDraft("whiteboard", "Whiteboard/Board.mesa-board.json", WHITEBOARD_KEY);
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const active = useRef<{ color: string; points: Array<[number, number]> } | null>(null);
  const paintGeneration = useRef(0);
  const [color, setColor] = useState("#e0a948");
  let board: BoardDocument = { version: 1, strokes: [] };
  let parseError = "";
  try { board = parseBoardDocument(draft.content); } catch (error) { parseError = String(error); }
  ...
  const paint = () => {
    ...
    const draw = () => {
      if (generation !== paintGeneration.current) return;
      for (const stroke of [...boardRef.current.strokes, ...(active.current ? [active.current] : [])]) {
        if (!stroke.points.length) continue;
        ctx.beginPath(); ctx.moveTo(...stroke.points[0]);
        for (const point of stroke.points.slice(1)) ctx.lineTo(...point);
        ctx.strokeStyle = stroke.color; ctx.lineWidth = 2.5; ctx.lineCap = "round"; ctx.lineJoin = "round"; ctx.stroke();
      }
    };
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    const background = boardRef.current.background;
    if (!background) { draw(); return; }
    const image = new Image();
    image.onload = () => {
      if (generation !== paintGeneration.current) return;
      ctx.clearRect(0, 0, canvas.width, canvas.height); ctx.drawImage(image, 0, 0); draw();
    };
  ...
  useEffect(() => {
    const canvas = canvasRef.current!;
    const resize = () => {
      canvas.width = Math.max(canvas.parentElement!.clientWidth, 1);
      canvas.height = Math.max(canvas.parentElement!.clientHeight, 1);
      paint();
    };
    resize();
    const observer = new ResizeObserver(resize); observer.observe(canvas.parentElement!);
    return () => { observer.disconnect(); ++paintGeneration.current; };
  }, []);
  useEffect(() => { paint(); }, [draft.content]);
  const at = (e: React.PointerEvent): [number, number] => {
    const r = canvasRef.current!.getBoundingClientRect(); return [e.clientX - r.left, e.clientY - r.top];
  };
  const down = (e: React.PointerEvent) => {
    if (e.button !== 0 || parseError) return;
    // Mark dirty on pointer-down, before any pending initial read can arrive.
    active.current = { color, points: [at(e)] };
    draft.edit(JSON.stringify(boardRef.current));
    e.currentTarget.setPointerCapture?.(e.pointerId);
  };
  const move = (e: React.PointerEvent) => {
    if (!active.current) return;
    active.current.points.push(at(e)); paint();
  };
  const up = () => {
    if (!active.current) return;
    const next = { ...boardRef.current, strokes: [...boardRef.current.strokes, active.current] };
    active.current = null; boardRef.current = next; draft.edit(JSON.stringify(next));
  };
  // Closing or switching vaults can unmount the canvas before pointer-up.
  // Retain that final partial stroke in the originating local draft.
  const finishStroke = useRef(up);
  finishStroke.current = up;
  useEffect(() => () => finishStroke.current(), []);
  const clear = () => {
    if (parseError || !window.confirm("Clear this whiteboard draft? The saved vault document remains available until you save again.")) return;
    active.current = null; ++paintGeneration.current;
    draft.edit(JSON.stringify({ version: 1, strokes: [] }));
  };
  const COLORS = ["#e0a948", "#7259cc", "#3cae8b", "#ff6b6b", "#cfd6e6"];
  return <div className="ov-board">
    <div className="ov-board-bar">
      {COLORS.map(c => <button key={c} className={"ov-swatch" + (color === c ? " on" : "")}
        style={{ background: c }} onClick={() => setColor(c)} aria-label={"Pen " + c} />)}
      <button className="btn ghost" disabled={!!parseError || board.strokes.length === 0} onClick={() => { active.current = null; draft.edit(JSON.stringify({ ...boardRef.current, strokes: boardRef.current.strokes.slice(0, -1) })); }}>Undo stroke</button>
      <button className="btn ghost" onClick={clear} disabled={!!parseError}>Clear</button>
      <button className="btn ghost ov-save-vault" disabled={!draft.ready || draft.saving || !draft.root || !!parseError}
        onClick={() => { up(); void draft.save(); }}>{draft.saving ? "Saving…" : "Save editable board"}</button>
    </div>
    {draft.status && <p className="ov-vault-status" role="status">{draft.status}</p>}
    {(draft.error || parseError) && <p className="ov-local-warn" role="alert">{draft.error || parseError}</p>}
    <div className="ov-board-canvas"><canvas aria-label="Whiteboard drawing" ref={canvasRef}
      onPointerDown={down} onPointerMove={move} onPointerUp={up} onPointerCancel={up} onLostPointerCapture={up} /></div>
  </div>;
}
      ...
      <section className="ov-settings-section">
        ...
        <div className="ov-setting-line">
          <div>
            ...
            <div className="setting-desc">Ask the system canvas to favor smoother graph motion when available.</div>
          </div>
          <OverlayToggle
...
export function Overlay() {
  ...
  const overlayWindowRequest = useAppStore((s) => s.overlayWindowRequest);
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      // fitWin reserves OVERLAY_BOTTOM below its effective viewport. Shift
      // that viewport upward when the dock wraps so every projected window,
      // including Pi's downward composer, ends above the actual dock.
      setSafeViewportHeight(
        Math.min(window.innerHeight, Math.max(OVERLAY_BOTTOM + 220, dockTop - 8 + OVERLAY_BOTTOM))
      );
    };
    measure();
  ...
  // Store-level callers can request a specific overlay window (with optional
  // geometry) from anywhere in the app: search routes here while the overlay
  // is open, and Pi's Deep Research wing can detach into this floating window.
  useEffect(() => {
    if (!overlayWindowRequest) return;
    const { id, rec } = overlayWindowRequest;
    setWins((w) => ({
      ...w,
      [id]: {
        ...w[id],
  ...
  useEffect(() => {
    if (wins.research.open && deepResearchSurface === "overlay") {
      openDeepResearch(false);
    }
  }, [deepResearchSurface, wins.research.open, openDeepResearch]);
  useEffect(() => {
    // Null means closed or an incoming overlay claim, not "another host owns
    // it"; only an explicit different owner may displace this stored window.
    if (!deepResearchSurface || deepResearchSurface === "overlay" || !wins.research.open) return;
    setWins((current) => ({
      ...current,
      research: { ...current.research, open: false },
    }));
  }, [deepResearchSurface, wins.research.open]);
  useEffect(() => {
    if (open) return;
    if (wins.research.open) {
      setWins((current) => ({
        ...current,
        research: { ...current.research, open: false },
      }));
    }
    if (deepResearchSurface === "overlay") setDeepResearchSurface(null);
  }, [deepResearchSurface, open, setDeepResearchSurface, wins.research.open]);
  useEffect(() => {
    if (!open) return;
  ...
  useEffect(() => {
    if (open) {
      ...
      const raf = requestAnimationFrame(() => {
        overlayRef.current?.querySelector<HTMLElement>(".ov-close")?.focus();
      });
      return () => cancelAnimationFrame(raf);
    }
  ...
  const resetOverlayLayout = () => {
    ...
    setOrder(["scratch", "whiteboard", "gallery", "settings", "search", "agent", "research", "tasks", "calendar"]);
  };
  ...
  return (
    ...
    >
      ...
      {(Object.keys(wins) as WinId[]).map((id) =>
        !wins[id].open || (id === "research" && deepResearchSurface !== "overlay") ? null : id === "agent" ? (
          <PiOverlayWindow
            key={id}
            rec={fitToViewport(wins[id], safeViewportHeight)}
          ...
          <FloatingWindow
            ...
            onChange={(p) => patch(id, p)}
            onClose={() => {
              if (id === "research") setDeepResearchSurface(null);
              toggle(id);
            }}
            onFocus={() => focus(id)}
          >
      ...
      <div className="ov-dock" ref={dockRef} data-native-webview-occluder="">
        {DOCK.map((d) => (
          <button
            key={d.id}
            className={
              "ov-dock-btn" +
              ((d.id === "research" ? deepResearchSurface === "overlay" : wins[d.id].open)
                ? " on"
                : "")
            }
            onClick={() => {
              if (d.id === "research") {
                if (deepResearchSurface === "overlay" && wins.research.open) {
                  setDeepResearchSurface(null);
                  patch("research", { open: false });
                } else {
                  openDeepResearch(false);
                  setDeepResearchSurface("overlay");
                  openWindow("research");
                }
                return;
              }
            ...
            title={d.label}
            aria-label={`${d.label} window`}
            aria-pressed={d.id === "research" ? deepResearchSurface === "overlay" : wins[d.id].open}
          >
            <span className="ov-dock-icon" aria-hidden="true">{d.icon}</span>
