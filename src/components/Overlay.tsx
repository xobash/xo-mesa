...
import { localNoteWriteMessage, readLocalNote, writeLocalNote } from "../lib/localNotes";
...
/** Per-day scratchpad with a date picker. Browser scratch space, not a vault
 *  note — see `lib/localNotes.ts` for why that distinction is surfaced. */
function Scratchpad() {
  const saveOverlayArtifact = useAppStore((s) => s.saveOverlayArtifact);
  ...
  const save = (v: string) => {
    // The text stays on screen either way; what changes is whether the user is
    // told it will survive a relaunch.
    setText(v);
    setSaveError(localNoteWriteMessage(writeLocalNote(SCRATCH_PREFIX + day, v)));
  };
  const saveIntoVault = async () => {
    setVaultStatus("");
    const rel = await saveOverlayArtifact("scratchpad", text, day);
    setVaultStatus(rel ? `Saved in vault as ${rel}.` : "Mesa could not save this item into the vault.");
  };
  return (
    <div className="ov-scratch-win">
      ...
      />
      <button className="btn ghost ov-save-vault" onClick={() => void saveIntoVault()}>
        Save into vault
      </button>
      {vaultStatus && <p className="ov-vault-status" role="status">{vaultStatus}</p>}
      {saveError && (
        <p className="ov-local-warn" role="alert">
          {saveError}
        </p>
      )}
    </div>
...
/** A persistent whiteboard. Browser scratch space, not a vault file — a board
 *  that outgrows its storage says so rather than quietly stopping saving. */
function Whiteboard() {
  const saveOverlayArtifact = useAppStore((s) => s.saveOverlayArtifact);
  ...
  const up = () => {
    ...
    // The stroke is already on the canvas; this decides whether it survives a
    // relaunch. A refused write leaves the LAST saved board intact, so the
    // warning is also the user's cue that the board on screen is ahead of it.
    setSaveError(
      localNoteWriteMessage(writeLocalNote(WHITEBOARD_KEY, canvasRef.current!.toDataURL()))
    );
  };
  const clear = () => {
    const c = canvasRef.current!;
    c.getContext("2d")!.clearRect(0, 0, c.width, c.height);
    // Clearing is also the remedy for a full board, so it must never be the
    // thing that fails: an empty string is always writable.
    setSaveError(localNoteWriteMessage(writeLocalNote(WHITEBOARD_KEY, "")));
  };
  const saveIntoVault = async () => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    ...
    try {
      ...
      const rel = await saveOverlayArtifact("whiteboard", bytes);
      setVaultStatus(rel ? `Saved in vault as ${rel}.` : "Mesa could not save this item into the vault.");
    } catch (error) {
      setVaultStatus(`Mesa could not prepare this board for the vault: ${String(error)}`);
    }
  };
