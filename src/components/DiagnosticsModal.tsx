...
import { startWorkloadRecording, stopWorkloadRecording, getWorkloadRecording, exportWorkloadRecording, summarizeWorkloadRecording } from "../lib/workloadRecording";
    ...
    <Modal onClose={() => setOpen(false)} className="diagnostics" title="Diagnostics">
      <header className="modal-head">
        ...
        <div className="modal-head-actions">
          <button className="btn" onClick={() => { setTick((n) => n + 1); setCacheRefresh((n) => n + 1); }}>Refresh</button>
          <button className="icon-btn" onClick={() => setOpen(false)} aria-label="Close">×</button>
        </div>
      </header>
      <div className="settings-body diag-body">
        <section className="diag-section">
          <h3>Record a mixed workload</h3>
          <p className="setting-desc">Record locally, then type while syncing, indexing, using a PDF, or working with detached windows. Reopen Diagnostics to stop and copy the measurements. Recording stops after ten minutes and contains no document text or paths.</p>
          <div className="diag-timeline">
            <button className="btn" disabled={!!getWorkloadRecording()?.running} onClick={() => {
              let detachedWindows: number | null = null;
              if (IN_TAURI) void import("@tauri-apps/api/window").then(({ getAllWindows }) => getAllWindows()).then(windows => { detachedWindows = Math.max(0, windows.length - 1); }).catch(() => {});
              startWorkloadRecording(IN_TAURI, () => {
                const state = useAppStore.getState();
                const work = backgroundWork.snapshot();
                return { sync: state.syncBusy, index: state.indexingTextFiles > 0 || (work.byKind.index?.active ?? 0) > 0,
                  pdf: !!document.querySelector('[data-testid="pdf-editor"]'), detachedWindows };
              });
              setRecordingMessage(""); setOpen(false);
            }}>Start recording</button>
            <button className="btn" disabled={!getWorkloadRecording()?.running} onClick={() => { stopWorkloadRecording(); setTick(value => value + 1); }}>Stop recording</button>
            <button className="btn" disabled={!getWorkloadRecording()} onClick={() => {
              void navigator.clipboard.writeText(exportWorkloadRecording()).then(() => setRecordingMessage("Measurements copied.")).catch(error => setRecordingMessage(`Could not copy measurements: ${String(error)}`));
            }}>Copy measurements</button>
            <span>{getWorkloadRecording()?.running ? "Recording" : "Not recording"} · {getWorkloadRecording()?.inputs.length ?? 0} input samples</span>
          </div>
          <p className="setting-desc">Input timing is a next-frame estimate from this window. Detached-window count is the count at recording start; record separately in each window. A workload with zero input samples is untested.</p>
          {recordingMessage && <p role="status">{recordingMessage}</p>}
          {getWorkloadRecording() && <table aria-label="Recorded input timing">
            <thead><tr><th>Workload</th><th>Inputs</th><th>Median</th><th>95th percentile</th></tr></thead>
            <tbody>{Object.entries(summarizeWorkloadRecording(getWorkloadRecording()!)).map(([name, summary]) =>
              <tr key={name}><th>{({ all: "All input", sync: "During sync", index: "During indexing", pdf: "With PDF editor", detached: "With detached windows" })[name]}</th><td>{summary.count}</td>
                <td>{summary.p50Ms === null ? "Untested" : `${summary.p50Ms.toFixed(1)} ms`}</td>
                <td>{summary.p95Ms === null ? "Untested" : `${summary.p95Ms.toFixed(1)} ms`}</td></tr>)}</tbody>
          </table>}
        </section>
        ...
        <section className="diag-section">
          ...
          <div className="diag-timeline">
            ...
            <button className="btn" onClick={() => void clearLocalIndexCache().then(() => setCacheRefresh((n) => n + 1))}>Clear local cache</button>
          </div>
        ...
        <section className="diag-section">
          ...
          <span className="setting-desc">
            Renderer trends cover only this window. Missing heap samples mean incomplete evidence. Native readings cover identified descendants; shared WebKit, GPU and other system-hosted processes may be absent. These figures do not establish a whole-app memory plateau.
          </span>
