...
export function TopBar() {
  ...
  const moveViewToCenter = useAppStore((s) => s.moveViewToCenter);
  ...
  const shows = (v: "preview" | "graph" | "tasks") =>
    centerView === v || (!narrowWorkspace && rightStack.includes(v));

  const toggleVisiblePanel = (panel: "preview" | "graph" | "tasks") => {
    if (narrowWorkspace) {
      if (centerView === panel) removeViewFromWorkspace(panel);
      else moveViewToCenter(panel);
      return;
    }
    togglePanel(panel);
  };
  ...
  const onPanelKeyDown = (
    ...
    panel: "preview" | "graph" | "tasks"
  ) => {
    if (e.repeat || (e.key !== "Enter" && e.key !== " ")) return;
    e.preventDefault();
    toggleVisiblePanel(panel);
  };
    ...
    <header className="topbar">
      ...
      <div className="topbar-right">
        ...
        <button
          ...
          aria-pressed={shows("tasks")}
          title="Toggle the Tasks panel · drag onto the right to dock, or away to pop out"
          onPointerDown={(e) => startViewDrag("tasks", "handle", e, () => toggleVisiblePanel("tasks"))}
          onKeyDown={(e) => onPanelKeyDown(e, "tasks")}
        >
          Tasks
        </button>
        <div className="seg">
          ...
          aria-pressed={shows("preview")}
          title="Toggle the Preview panel · drag to dock or pop out"
            onPointerDown={(e) =>
              startViewDrag("preview", "handle", e, () => toggleVisiblePanel("preview"))
            }
            onKeyDown={(e) => onPanelKeyDown(e, "preview")}
          >
            Preview
          </button>
          <button
          className={"seg-btn pane-handle" + (shows("graph") ? " on" : "")}
          aria-pressed={shows("graph")}
          title="Toggle the Graph panel · drag to dock or pop out"
            onPointerDown={(e) =>
              startViewDrag("graph", "handle", e, () => toggleVisiblePanel("graph"))
            }
            onKeyDown={(e) => onPanelKeyDown(e, "graph")}
          >
            Graph
          </button>
        </div>
