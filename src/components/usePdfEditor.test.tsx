...
vi.mock("pdfjs-dist", () => {
  ...
  return {
    ...
    PDFWorker: class {
      destroyed = false;
      // The real PDFWorker resolves this once the worker thread has booted.
      // Mesa pre-warms workers off that promise, so the double has to have it.
    ...
    getDocument: (params?: { worker?: unknown }) => {
      ...
      const createLoadingTask = <T,>(promise: Promise<T>) => {
        let destroyed = false;
        pdfjsState.loadingTasksCreated++;
        return {
          promise,
          destroy: () => {
            if (!destroyed) {
              destroyed = true;
              pdfjsState.loadingTasksDestroyed++;
            }
      ...
      if (pdfjsState.failNextParse) {
        ...
        return createLoadingTask(Promise.reject(new Error("corrupt document")));
      }
      const makeDoc = () => ({
        get numPages() {
      ...
      if (!pdfjsState.gateDocument) return createLoadingTask(Promise.resolve(makeDoc()));
...
afterEach(async () => {
  ...
  pdfjsState.failNextParse = false;
  pdfjsState.loadingTasksCreated = 0;
  pdfjsState.loadingTasksDestroyed = 0;
  latest = null;
  ...
  // The warm-spare slot is module state that outlives a test. A worker left in
  // it would be adopted by the next test instead of being constructed there,
  // making worker counts depend on test order.
  clearPdfWorkerSpare();
...
describe("pdf.js worker lifecycle", () => {
  it("destroys a pending loading task when the PDF is replaced", async () => {
    const a = await makePdf("W-pending-a");
    const b = await makePdf("W-pending-b");
    fsState.files.set("/w-pending-a.pdf", a);
    fsState.files.set("/w-pending-b.pdf", b);
    pdfjsState.gateDocument = true;
    host = document.createElement("div");
    document.body.append(host);
    root = createRoot(host);

    await render(file("/w-pending-a.pdf"));
    await waitFor(() => pdfjsState.loadingTasksCreated === 1);

    // Let the replacement document complete normally. The first task must be
    // destroyed by the parse effect cleanup before the second parse starts.
    pdfjsState.gateDocument = false;
    await render(file("/w-pending-b.pdf"));
    await waitFor(() => !!latest?.bytes && pdfBytesEqual(latest.bytes, b));
    await waitFor(() => pdfjsState.loadingTasksCreated === 2);

    expect(pdfjsState.loadingTasksDestroyed).toBe(1);
  });

  it("destroys a pending loading task on unmount", async () => {
    const doc = await makePdf("W-pending-unmount");
    fsState.files.set("/w-pending-unmount.pdf", doc);
    pdfjsState.gateDocument = true;
    host = document.createElement("div");
    document.body.append(host);
    root = createRoot(host);

    await render(file("/w-pending-unmount.pdf"));
    await waitFor(() => pdfjsState.loadingTasksCreated === 1);
  ...
  it("adopts a warm spare instead of booting a worker on the open path", async () => {
    ...
    // The spare's own construction is not part of what the open costs.
    pdfjsState.workersCreated = 0;

    host = document.createElement("div");
    document.body.append(host);
    root = createRoot(host);
    await render(file("/w-warm.pdf"));
    await waitFor(() => pdfjsState.workersUsed.length >= 1);

    // Worker boot was 145 ms of a 286 ms open. Opening must spend none of it
    // when a warm, never-used worker is already sitting in the pool.
    expect(pdfjsState.workersCreated).toBe(0);
    expect(pdfjsState.workersUsed[0]).toBe(spare);
  });

  it("never returns a worker that has parsed a document to the shared pool", async () => {
    const doc = await makePdf("W-used");
    fsState.files.set("/w-used.pdf", doc);
    host = document.createElement("div");
    document.body.append(host);
    root = createRoot(host);

    await render(file("/w-used.pdf"));
    await waitFor(() => latest?.pageCount === 1);
    const used = pdfjsState.workersUsed[0];
    ...
    // A worker that has held document state must die with its viewer, so one
    // wedged document can never reach another viewer's PDF. Anything the pool
    // offers afterwards has to be a different, freshly booted worker.
    const next = takePdfWorker();
    expect(next).not.toBe(used);
  });

  it("gives back a worker it never parsed with, so a fast remount is free", async () => {
    // A viewer that mounts and leaves before any parse (rapid tab switching)
    // still holds a fully booted, never-used worker. Destroying it would make
    // the next open pay for a boot that has already been paid for.
    const doc = await makePdf("W-untouched");
    fsState.files.set("/w-untouched.pdf", doc);
    host = document.createElement("div");
    document.body.append(host);
    root = createRoot(host);

    // Fail the read, so the worker is claimed (the open path claims it up
    // front) but no document is ever handed to it.
    fsState.onRead = () => {
      throw new Error("read never completed");
    };
    await render(file("/w-untouched.pdf"));
    await waitFor(() => pdfjsState.workersCreated === 1);
    expect(pdfjsState.workersUsed.length).toBe(0);
    ...
    expect(pdfjsState.workersDestroyed).toBe(0);
    expect(takePdfWorker()).toBeTruthy();
  });
});

describe("files that carry a .pdf name but are not PDFs", () => {
  /** What a saved "download failed" page looks like on disk: HTML, with the
   *  leading newline a real vault's copies actually have. */
  const htmlSavedAsPdf = () =>
    new TextEncoder().encode("\n<!DOCTYPE html>\n<html><body>Not found</body></html>\n");

  it("keeps its never-used worker when Mesa's own byte check rejects the file", async () => {
    // This is not an exotic case. In the vault this was measured against, 880 of
    // 1,629 `.pdf` files are not PDFs — 872 of them the same saved HTML error
    // page. Destroying the worker here made the NEXT real PDF pay a full boot:
    // parse 3.5 ms -> 41 ms, first page 121 ms -> 170 ms.
    const good = await makePdf("W-after-nonpdf");
    fsState.files.set("/not-really.pdf", htmlSavedAsPdf());
    fsState.files.set("/really.pdf", good);
    host = document.createElement("div");
    document.body.append(host);
    root = createRoot(host);

    await render(file("/not-really.pdf"));
    await waitFor(() => latest?.renderError === true);

    // pdf.js was never handed these bytes, so the worker is still virgin and
    // nothing about it is in doubt. Blaming it for Mesa's own rejection is what
    // threw away the boot.
    expect(pdfjsState.workersUsed.length).toBe(0);
    expect(pdfjsState.workersDestroyed).toBe(0);
    expect(pdfjsState.workersCreated).toBe(1);

    // …and the real PDF that follows reuses it rather than booting another.
    await render(file("/really.pdf"));
    await waitFor(() => latest?.pageCount === 1);
    expect(pdfjsState.workersCreated).toBe(1);
    expect(pdfjsState.workersUsed.length).toBe(1);
  });

  it("hands that untouched worker back to the pool on unmount", async () => {
    fsState.files.set("/not-really.pdf", htmlSavedAsPdf());
    host = document.createElement("div");
    document.body.append(host);
    root = createRoot(host);
  ...
  it("stops claiming the file is loading once the open has failed", async () => {
    // The fallback explains what the file actually is; a "Loading PDF..." line
    // pinned above it says work is still in flight when none is.
    fsState.files.set("/not-really.pdf", htmlSavedAsPdf());
    host = document.createElement("div");
    document.body.append(host);
    root = createRoot(host);
...
describe("memory behaviour on the open path", () => {
  ...
  it("keeps edited bytes on their own copy, never aliasing the transform result", async () => {
    ...
    // A transform can retain its result. Later writes through that reference
    // must not change the adopted bytes, history, or the next verified save.
    edited.fill(0);
    expect(pdfBytesEqual(latest!.bytes, expected)).toBe(true);
    expect(pdfBytesEqual(fsState.files.get("/alias.pdf") ?? null, start)).toBe(true);
    await act(async () => { await latest!.undo(); });
    expect(pdfBytesEqual(latest!.bytes, start)).toBe(true);
    expect(latest!.dirty).toBe(false);
    expect(latest!.canRedo).toBe(true);
    await act(async () => { await latest!.redo(); });
    expect(pdfBytesEqual(latest!.bytes, expected)).toBe(true);
    expect(latest!.dirty).toBe(true);
    expect(latest!.canUndo).toBe(true);
    await act(async () => { await latest!.save(); });
    expect(pdfBytesEqual(fsState.files.get("/alias.pdf") ?? null, expected)).toBe(true);
    expect(latest!.dirty).toBe(false);
  });
});
