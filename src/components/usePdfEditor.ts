  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // path: in a real vault more than half of the `.pdf` files can be saved
        // HTML error pages, and each one used to cost the NEXT real open a full
        // worker boot (measured +42 ms to first page).
        ...
        // From here the worker is no longer virgin: it owns document state, so
        // it can never go back to the shared spare slot.
