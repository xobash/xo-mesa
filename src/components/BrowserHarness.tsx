...
function visibleOccluderRects(
  ...
  const rects: HarnessRect[] = [];
  for (const candidate of candidates) {
    if (!candidate.isConnected || candidate.contains(slot) || slot.contains(candidate)) {
      continue;
    }
    ...
    rects.push(roundRect(candidate.getBoundingClientRect()));
  }
  return rects;
}
...
export function BrowserHarness({
  ...
  const nativeNavigate = async (url: string, attempt = 0): Promise<void> => {
    ...
    const visible = slot
      ? !harnessOccluded(
          rect,
          visibleOccluderRects(
            slot,
            Array.from(
              document.querySelectorAll<HTMLElement>(NATIVE_WEBVIEW_OCCLUDER)
            )
          )
        )
      : true;
    try {
      await invoke("harness_navigate", {
        url,
        ...rect,
      ...
      setNativeOk(true);
      nativeOccludedRef.current = !visible;
      setNativeOccluded(!visible);
      setMode("native");
    } catch (e) {
      if (attempt < 1) {
  ...
  // event-driven: a permanent rAF loop used to force layout reads at 60 Hz
  // while an otherwise idle browser was open.
  ...
  useEffect(() => {
    ...
    let retry = 0;
    let animating = false;
    let confirmedBounds: HarnessRect | null = null;
    let boundsInFlight = false;
    let confirmedVisible: boolean | null = !nativeOccludedRef.current;
    let visibilityInFlight: Promise<unknown> | null = null;
    let lastOccluded = nativeOccludedRef.current;
    let occluders = Array.from(
      document.querySelectorAll<HTMLElement>(NATIVE_WEBVIEW_OCCLUDER)
    );
    ...
    const schedule = () => {
      if (!raf) raf = window.requestAnimationFrame(syncGeometry);
    };
    const observer = new MutationObserver(() => { refreshOccluders(); schedule(); });
    observer.observe(document.body, {
      childList: true,
      subtree: true,
      attributes: true,
      attributeFilter: ["data-native-webview-occluder"],
    });

    const requestVisibility = (visible: boolean) => {
      if (visibilityInFlight || confirmedVisible === visible) return;
      const requested = visible;
      visibilityInFlight = invoke("harness_visibility", { visible: requested })
        .then(() => {
          confirmedVisible = requested;
        })
        .catch(() => {
          // A later geometry signal retries; no idle frame loop is needed.
        })
        .finally(() => {
    ...
    const syncGeometry = () => {
      ...
      if (el) {
        const rect = roundRect(el.getBoundingClientRect());
        const usable = rectUsable(rect);
        const occluded =
          usable && harnessOccluded(rect, visibleOccluderRects(el, occluders));
        if (lastOccluded !== occluded) {
          lastOccluded = occluded;
          nativeOccludedRef.current = occluded;
          setNativeOccluded(occluded);
        }
        requestVisibility(usable && !occluded);

        if (usable && !boundsInFlight && rectsDiffer(confirmedBounds, rect)) {
          lastSentRef.current = rect;
          boundsInFlight = true;
          invoke("harness_bounds", { ...rect, viewportH: window.innerHeight })
            .then(() => {
              confirmedBounds = rect;
            })
            .catch(() => {
              window.clearTimeout(retry);
              retry = window.setTimeout(schedule, 250);
            })
            .finally(() => {
    ...
    const animate = () => {
      syncGeometry();
      if (animating) raf = window.requestAnimationFrame(animate);
    };
    ...
    const stopAnimation = () => { animating = false; schedule(); };
    const slot = frameSlotRef.current;
    const resizeObserver = typeof ResizeObserver === "undefined" || !slot
      ? null
      : new ResizeObserver(schedule);
    if (resizeObserver && slot) resizeObserver.observe(slot);
    window.addEventListener("resize", schedule);
    window.addEventListener("scroll", schedule, true);
    window.addEventListener("pointermove", schedule, true);
    document.addEventListener("transitionrun", startAnimation, true);
    document.addEventListener("transitionend", stopAnimation, true);
    document.addEventListener("transitioncancel", stopAnimation, true);
    schedule();
    return () => {
      observer.disconnect();
      ...
      document.removeEventListener("transitionrun", startAnimation, true);
      document.removeEventListener("transitionend", stopAnimation, true);
      document.removeEventListener("transitioncancel", stopAnimation, true);
      nativeOccludedRef.current = false;
      setNativeOccluded(false);
      const hide = () => {
        void invoke("harness_visibility", { visible: false }).catch(() => {});
      };
      if (visibilityInFlight) void visibilityInFlight.finally(hide);
      else hide();
    };
  }, [mode]);
  ...
  return (
    <section className="agent-browser">
      ...
      <div
        ref={frameSlotRef}
        data-native-webview-occluded={nativeOccluded ? "true" : undefined}
        className={
      ...
      >
        {mode === "native" && !showStart ? (
          ...
          <div className="agent-browser-native-host" aria-hidden="true">
            <span>
              {nativeOccluded
                ? "Live page hidden while another Mesa surface is in front."
                : "Loading page in the native webview…"}
            </span>
          </div>
        ) : (
          <iframe
