 ...
 * Loading starts only when markdown actually renders. The app shell imports
 * `Preview` for the empty default workspace, and module-scope prefetch pulled
 * this optional renderer into sessions that had not opened a note yet.
 */
...
function loadRenderer(): Promise<void> {
  if (!rendererReady) {
    rendererReady = import("../lib/markdown").then((m) => {
...
export function MarkdownView({
  ...
  useEffect(() => {
    if (ready) return;
    let alive = true;
    void loadRenderer().then(() => alive && setReady(true));
    return () => {
      alive = false;
    };
  }, [ready]);
