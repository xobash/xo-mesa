...
import { extractLinks, extractLinksAndFirstImage, extractTags, extractAliases } from "./lib/markdownExtract";
import {
  bumpActivityAmount,
  changedLineStats,
  changedSnippet,
...
export interface AppState {
  ...
  setSettingsSection: (section: "settings" | "history" | "deleted") => void;
  openRecovery: () => void;
...
export const useAppStore = create<AppState>((set, get) => {
  ...
  return {
    ...
    addPeer: (input, name, fingerprint) => {
      ...
      commitSettings({ ...get().settings, peers: [...peers, peer],
    ...
    removePeer: async (id) => {
      ...
      await reportingFailure("Remove sync peer", async () => {
        ...
        commitSettings({ ...get().settings, peers: get().settings.peers.filter(peer => peer.id !== id),
    ...
    toggleListen: async () => {
      ...
      try {
        ...
        await retireSyncPeers(s.vaultPath, s.settings.retiredSyncPeers ?? []);
        if (get().vaultPath !== s.vaultPath || generation !== vaultOpenGeneration || !get().settings.syncEnabled) return;
    ...
    ...createSyncWorkflow({
      ...
      transfer: (root, address, token, fingerprint, retry, admitted) =>
        import("./lib/sync").then(async ({ syncWithPeer, retireSyncPeers }) => {
          ...
          return syncWithPeer(root, address, token, fingerprint, retry);
        }),
    ...
    restoreTextRevision: async (revisionId) => {
      ...
      const generation = vaultOpenGeneration;
      const current = () => get().vaultPath === root && get().activePath === active && vaultOpenGeneration === generation;
      await textSaves.flushAll();
    ...
    getVaultGeneration: () => vaultOpenGeneration,
    saveOverlayArtifact: async (kind, content, date, binding) => {
      ...
      if (saved.isMarkdown) {
        ...
        } else addNote(saved, String(content));
      } else {
        const existing = get().files.some((file) => file.relPath === saved!.relPath);
