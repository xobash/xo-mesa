...
interface Dependencies {
  ...
  transfer(root: string, address: string, token: string, fingerprint: string | null, retry: string[] | null, admitted: () => boolean): Promise<SyncReport>;
  ...
  async function run(peerId: string): Promise<void> {
    ...
    try {
      ...
      op.transfer = backgroundWork.enqueue('sync', async () => {
        ...
        return deps.transfer(root, peer.address, token, peer.fingerprint ?? null, retryRels.length ? retryRels : null, admitted);
      });
