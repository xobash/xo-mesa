...
describe("Windows run.cmd bootstrap contract", () => {
  ...
  it("rejects a present but unsupported WebView2 runtime", () => {
    const probe = runCmd.slice(runCmd.indexOf("\n:has_webview2"));
    expect(runCmd).toContain('set "MESA_WEBVIEW2_MIN_MAJOR=111"');
    expect(probe).toContain('call :webview2_version_ok "%%C"');
    expect(probe).toContain(":webview2_version_ok");
    expect(probe).toContain("LSS %MESA_WEBVIEW2_MIN_MAJOR%");
  });
...
it("enforces the same minimum runtime in both Windows installer formats", () => {
  const config = windowsConfig;
  expect(config.bundle.windows.minimumWebview2Version).toBe("111.0.0.0");
  expect(config.bundle.targets).toEqual(expect.arrayContaining(["msi", "nsis"]));
});
