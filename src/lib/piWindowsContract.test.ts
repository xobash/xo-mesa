...
import libRsSrc from "../../src-tauri/src/lib.rs?raw";
...
describe("Windows Pi launcher contract", () => {
  ...
  it("kills the Pi PTY child when Mesa exits instead of orphaning it", () => {
    // The PTY child (`pi`, and the Node process behind it) is an ordinary child
    // process, not a job-object member, and Tauri does not drop managed state
    // on exit — so nothing reaped it when the event loop ended. On Windows the
    // orphan keeps a handle on the vault folder, which is how a later
    // `run.cmd` ends up fighting a locked build cache.
    expect(terminalSrc).toContain("pub fn stop_all_sessions");
    expect(terminalSrc).toContain("sessions.drain()");
    expect(terminalSrc).toContain("session.child.try_wait()");
    expect(terminalSrc).toContain("mesa-terminal-reaper");
    expect(terminalSrc).toContain("let _ = child.wait()");
    // Recovered from rather than skipped: a panicked reader thread must not be
    // the reason a child process survives.
    expect(terminalSrc).toContain("unwrap_or_else(|poisoned| poisoned.into_inner())");

    expect(libRsSrc).toContain("tauri::RunEvent::Exit");
    expect(libRsSrc).toContain("terminal::stop_all_sessions");
    // The loopback activity server's thread and bound port go with it.
    expect(libRsSrc).toContain("activity::activity_stop()");
    // `.run(context)` swallows the event loop; only the built-app form can see
    // `RunEvent::Exit` at all.
    expect(libRsSrc).toContain(".build(tauri::generate_context!())");
    expect(libRsSrc).not.toContain(".run(tauri::generate_context!())");
  });
});
