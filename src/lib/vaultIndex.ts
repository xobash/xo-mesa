...
export function retainedSnapshotMetadata(
  entries: readonly SnapshotMetadata[],
  budget = GLOBAL_INDEX_CACHE_BUDGET_BYTES,
): SnapshotMetadata[] {
  let used = entries.reduce((total, entry) => total + entry.bytes, 0);
  const retained = [...entries];
  for (const old of [...retained].sort((a, b) => a.cachedAt - b.cachedAt)) {
    if (used <= budget || retained.length === 1) break;
    const index = retained.indexOf(old);
...
const database = (): Promise<IDBDatabase> => new Promise((resolve, reject) => {
  ...
  request.onupgradeneeded = () => {
    ...
    const metadata = request.result.createObjectStore('metadata', { keyPath: 'root' });
...
export async function vaultIndexStorageStats(): Promise<VaultIndexStorageStats> {
  try {
    ...
    try { return await new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        const snapshots = (request.result as SnapshotMetadata[]).filter(value => value?.root && Number.isFinite(value.bytes));
        resolve({ bytes: snapshots.reduce((total, value) => total + value.bytes, 0), snapshots: snapshots.length, budgetBytes: GLOBAL_INDEX_CACHE_BUDGET_BYTES });
      };
...
function valid(entry: RecordEntry): boolean {
  return !!entry && typeof entry.digest === 'string' && /^[a-f0-9]{64}$/.test(entry.digest) &&
    ...
    !!entry.note && typeof entry.note.title === 'string' && [entry.note.rawLinks, entry.note.rawReferences, entry.note.tags, entry.note.aliases].every(a => Array.isArray(a) && a.every(x => typeof x === 'string'));
}
...
export async function loadIndexedNotes(options: {
  ...
  // Every textual document can contain vault links. Keep them in the first
  // graph index; non-link-bearing search-only hydration is handled separately
  // by the store after this authoritative note pass.
