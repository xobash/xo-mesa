...
export interface ResourcePlateau {
  ...
  trend: "warming" | "plateau" | "growing" | "incomplete";
}
...
export function summarizeResourcePlateau(
  ...
  const completeHeap = history.every(sample => sample.heapBytes !== null && Number.isFinite(sample.heapBytes) && sample.heapBytes >= 0);
  const heapDeltaBytes =
    completeHeap && first.heapBytes !== null && last.heapBytes !== null
  ...
  const trend =
    history.length < WARMUP_SAMPLES
      ...
      : positiveHeapGrowth || positiveOwnedGrowth
        ? "growing"
        : completeHeap ? "plateau" : "incomplete";
  return {
