...
describe("LatestWinsQueue", () => {
  it("finishes active work and skips every stale queued interaction", async () => {
    const queue = new LatestWinsQueue<string, string>(1, true);
  ...
  it("settles superseded queued work without waiting for the active task", async () => {
    const queue = new LatestWinsQueue<string, string>(1, true);
    let finishActive!: (value: string) => void;
    ...
    const staleRun = vi.fn(async () => "stale");
    const stale = queue.enqueue("stale", staleRun);
    const staleResult = stale.catch((error) => error);

    queue.enqueue("latest", async () => "latest");

    await expect(staleResult).resolves.toBeInstanceOf(SupersededTaskError);
    expect(staleRun).not.toHaveBeenCalled();
    finishActive("active");
    await expect(active).resolves.toBe("active");
  });
