...
describe("normalizeVaultRelPath", () => {
  ...
  it("maps Windows backslash paths against a forward-slash root", () => {
    expect(
      normalizeVaultRelPath("C:\\MesaVault\\Notes\\idea.md", "C:/MesaVault")
    ).toBe("Notes/idea.md");
  });
  ...
  it("accepts Windows drive-letter file URLs", () => {
    expect(
      normalizeVaultRelPath("file:///C:/MesaVault/Notes/idea.md", "C:/MesaVault")
    ).toBe("Notes/idea.md");
  });
...
describe("canonicalRoot", () => {
  it("normalizes slashes and trailing separators", () => {
    expect(canonicalRoot("C:\\MesaVault\\")).toBe("C:/MesaVault");
  });
  ...
  it("leaves POSIX paths untouched", () => {
    expect(canonicalRoot("/tmp/MesaVault")).toBe("/tmp/MesaVault");
    expect(canonicalRoot("mesa://demo")).toBe("mesa://demo");
  });
});

describe("renameVaultFile contract", () => {
  it("uses the native no-replace operation instead of a check-then-rename race", () => {
    const start = vaultSource.indexOf("export async function renameVaultFile(");
    const end = vaultSource.indexOf("// --- drag-and-drop import", start);
    const body = vaultSource.slice(start, end);
    expect(body).toContain("caseOnlyRename");
    expect(body).toContain("srcRel.toLowerCase() === destRel.toLowerCase()");
    expect(body).toContain('invoke("vault_rename_no_replace"');
    expect(body).toContain("caseOnly: caseOnlyRename");
    expect(body).not.toContain("await exists(");
    expect(body).not.toContain("await rename(");
  });
});
...
describe("resolveTextVaultLink", () => {
  const files = [
    { ...demoFile("Notes/Plan.md", "md", true), name: "Plan" },
    { ...demoFile("Notes/Plan.txt", "txt"), name: "Plan" },
    { ...demoFile("Notes/Log.txt", "txt"), name: "Log" },
    { ...demoFile("assets/Log.png", "png"), name: "Log" },
  ];

  it("opens an exact text-file target without changing its extension", () => {
    expect(resolveTextVaultLink(files, "Notes/Log.txt")?.relPath).toBe("Notes/Log.txt");
  });

  it("uses a Markdown target first only when a bare name is ambiguous", () => {
    expect(resolveTextVaultLink(files, "Plan")?.relPath).toBe("Notes/Plan.md");
  });

  it("accepts a bare name for a non-Markdown text file", () => {
    expect(resolveTextVaultLink(files, "Log")?.relPath).toBe("Notes/Log.txt");
  });

  it("does not resolve a binary file as a text link", () => {
    expect(resolveTextVaultLink(files, "assets/Log.png")).toBeUndefined();
  });
});
