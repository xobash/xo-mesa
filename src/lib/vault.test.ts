...
describe("renameVaultFile contract", () => {
  it("uses a temporary same-folder hop for case-only renames", () => {
    ...
    expect(body).toContain("await rename(srcAbs, tempAbs)");
    expect(body).toContain("await rename(tempAbs, destAbs)");
    expect(body).toContain("await rename(tempAbs, srcAbs).catch");
  });
});
