...
// all. These source assertions supplement behavioral store and close-guard tests.
import store from "../store.ts?raw";
import app from "../App.tsx?raw";
import closeGuardSource from "./closeGuard.ts?raw";
...
describe("new-file writes: never replace a concurrent target", () => {
  it("uses missing-target preconditions for create, copy, and import", () => {
    const create = vault.slice(
      vault.indexOf("export async function createNote("),
      vault.indexOf("export async function writeVaultTextFile(")
    );
    expect(create).toContain("expectedMissing = true");
    expect(create).toContain(
      "expectedCurrentBytes: expectedMissing ? null : undefined"
    );

    const copy = vault.slice(
      vault.indexOf("export async function copyVaultFile("),
      vault.indexOf("export async function renameVaultFile(")
    );
    expect(copy).toContain("expectedCurrentBytes: null");

    const place = vault.slice(
      vault.indexOf("async function placeFile("),
      vault.indexOf("async function importZip(")
    );
    expect(place.match(/expectedCurrentBytes: null/g)).toHaveLength(2);
  });
});

describe("text-write coordinator: crash-safety flush", () => {
  const flush = storeAction("flushSave");

  it("flushes dirty paths only instead of rewriting the active file", () => {
    expect(flush).toContain("textSaves.flushAll()");
    expect(flush).not.toContain("writeNote(");
    expect(flush).not.toContain("activePath");
  });

  it("passes the first dirty baseline into the verified text write", () => {
    const coordinator = store.slice(
      store.indexOf("const textSaves = new TextSaveCoordinator"),
      store.indexOf("let vaultOpenGeneration")
    );
    const decision = coordinator.indexOf("flushableNoteText(file, content)");
    const write = coordinator.indexOf(
      "writeNote(file, pending, expectedContent)"
    );
    expect(decision).toBeGreaterThan(-1);
    expect(write).toBeGreaterThan(decision);
  });

  it("tracks failed paths independently and clears each issue after success", () => {
    const coordinator = store.slice(
      store.indexOf("const textSaves = new TextSaveCoordinator"),
      store.indexOf("let vaultOpenGeneration")
    );
    expect(coordinator).toContain("recordTextSaveIssue(key, target.relPath");
    expect(coordinator).toContain("clearTextSaveIssues([key])");
    expect(store).toContain("textSaveIssues: Record<string, TextSaveIssue>");
  });

  it("connects the main native window to the behavioral close guard", () => {
    expect(app).toContain("stop = installCloseGuard(");
    expect(app).toContain("() => getStore().flushSave()");
    expect(app).toContain("() => getStore().textSaveState.pending > 0");
    expect(closeGuardSource).toContain("Close safety failed to start:");
  });
});

describe("text-write coordinator: non-editor mutations", () => {
  it("routes calendar and task updates through the same verified queue", () => {
    const section = store.slice(
      store.indexOf("    addCalendarEvent: async"),
      store.indexOf("    importDropped: async")
    );
    expect(section).toContain("await saveTextMutation(file, content, prev)");
    expect(section).toContain("await saveTextMutation(existing, next, cur)");
    expect(section).toContain("await saveTextMutation(file, next, prev)");
    expect(section).not.toContain("writeNote(");
  });

  it("reloads calendar state when save recovery chooses the disk copy", () => {
    const start = store.indexOf("    useDiskTextForSaveIssue: async");
    const action = store.slice(start, store.indexOf("    newNote: async", start));
    expect(start).toBeGreaterThan(-1);
    expect(action).toContain("file.relPath === CALENDAR_FILE");
    expect(action).toContain("parseEvents(diskText)");
  });

  it("keeps active editors and external calendar state in sync", () => {
    const section = store.slice(
      store.indexOf("    addCalendarEvent: async"),
      store.indexOf("    importDropped: async")
    );
    expect(section).toContain("get().activePath === CALENDAR_FILE");
    expect(section).toContain("get().activePath === rel ? next : get().content");

    const watcher = store.slice(
      store.indexOf("  async function handleExternalChange("),
      store.indexOf("  async function setupWatcher(")
    );
...
describe("text-write gate: debounced editor save", () => {
  ...
  it("keys the dirty edit by the file's absolute path and disk baseline", () => {
    expect(setContent).toContain(
      "textSaves.markDirty(file.path, file, text, prev)"
    );
    expect(setContent).not.toContain("saveTimer");
    expect(store).not.toContain("let saveTimer");
  });

  it("resolves dirty paths before rename, delete, and vault switch", () => {
    const rename = store.slice(
      store.indexOf("    renameNote: async"),
      store.indexOf("    closeTab:")
    );
    expect(rename.indexOf("await textSaves.hold([file.path], true)")).toBeLessThan(
      rename.indexOf("await renameVaultFile(")
    );
    expect(rename.indexOf("saveHold.move(file.path, newFile.path, newFile)")).toBeGreaterThan(
      rename.indexOf("await renameVaultFile(")
    );

    const deletion = store.slice(
      store.indexOf("    deleteEntry: async"),
      store.indexOf("    renameNote: async")
    );
    expect(deletion.indexOf("await textSaves.hold(keys, false)")).toBeLessThan(
      deletion.indexOf("await removeFile(")
    );
    expect(deletion.indexOf("saveHold.discard()")).toBeGreaterThan(
      deletion.indexOf("await removeFile(")
    );

    const openStart = store.indexOf("    openVault: async");
    const opening = store.slice(
      openStart,
      store.indexOf("    listActiveTextRevisions:", openStart)
    );
    const flushes = [...opening.matchAll(/await textSaves\.flushAll\(\)/g)].map(
      (match) => match.index
    );
    expect(flushes).toHaveLength(2);
    expect(flushes[0]).toBeLessThan(
      opening.indexOf("++vaultOpenGeneration")
    );
    expect(flushes[1]).toBeLessThan(opening.indexOf("localStorage.setItem"));
    expect(flushes[1]).toBeGreaterThan(opening.indexOf("const recents ="));
  });

  it("does not retain a watcher that resolves after a vault switch", () => {
    const setup = store.slice(
      store.indexOf("  async function setupWatcher("),
      store.indexOf("  // External agents", store.indexOf("  async function setupWatcher("))
    );
    const open = store.slice(
      store.indexOf("    openVault: async"),
      store.indexOf(
        "    removeRecentVault:",
        store.indexOf("    openVault: async")
      )
    );
    expect(setup).toContain("const setupGeneration = ++watcherSetupGeneration");
    expect(setup).toContain("const stop = await watchVault");
    expect(setup).toContain("if (setupGeneration !== watcherSetupGeneration)");
    expect(setup).toContain("stop();");
    expect(open).toContain("watcherSetupGeneration++");
    expect(open).toContain("unwatch?.();");
  });

  it("scopes lazy reads and peeks to the vault-open generation", () => {
    const content = store.slice(
      store.indexOf("    ensureContent: async"),
      store.indexOf("    fileFor:", store.indexOf("    ensureContent: async"))
    );
    expect(content).toContain("const key = readKey(root, generation, relPath)");
    expect(content).toContain("vaultOpenGeneration === generation");
    expect(content).toContain("pendingContentReads.get(key)");
    expect(content).toContain("pendingPeekReads.get(key)");
    expect(content).toContain("pendingContentReads.get(key) === read");
    expect(content).toContain("pendingPeekReads.get(key) === read");
  });

  it("holds the dirty path through fail-closed disk-copy recovery", () => {
    const start = store.indexOf("    useDiskTextForSaveIssue: async");
    const recovery = store.slice(start, store.indexOf("    newNote:", start));
    const hold = recovery.indexOf("await textSaves.hold([key], false)");
    const read = recovery.indexOf("await readTextFileStrict(file.path)");
    const confirm = recovery.indexOf("window.confirm(");
    const discard = recovery.indexOf("saveHold.discard()");
    const refresh = recovery.indexOf("contentCache:", discard);
    expect(hold).toBeGreaterThan(-1);
    expect(read).toBeGreaterThan(hold);
    expect(confirm).toBeGreaterThan(read);
    expect(discard).toBeGreaterThan(confirm);
    expect(refresh).toBeGreaterThan(discard);
    expect(recovery).toContain("if (!confirmed)");
    expect(recovery).toContain("saveHold.release()");
    expect(recovery).toContain("saveHold?.release()");
  });

  it("never treats a save issue without a dirty entry as a successful retry", () => {
    const retryStart = store.indexOf("    retryTextSaveIssue: async");
    const retry = store.slice(
      retryStart,
      store.indexOf("    useDiskTextForSaveIssue: async", retryStart)
    );
    expect(retry).toContain("if (!textSaves.isDirty(key))");
    expect(retry).toContain("Retry unavailable");

    const diskStart = store.indexOf("    useDiskTextForSaveIssue: async");
    const disk = store.slice(diskStart, store.indexOf("    newNote: async", diskStart));
    expect(disk).not.toContain("if (!textSaves.isDirty(key))");
  });
});
...
describe("text-cache freshness: external modifications", () => {
  ...
  it("re-reads a cached textual file when it changes on disk", () => {
    ...
    const read = watcher.indexOf("readNoteResult(file)", decision);
    ...
    expect(watcher).toContain("if (!read.ok)");
    expect(watcher).toContain("keeping cached content");
    expect(queued).toBeGreaterThan(read);
    expect(watcher).toContain("next.contentCache = cache;");
  });

  it("leaves every dirty document alone and refreshes a clean active document", () => {
    expect(watcher).toContain("!textSaves.isDirty(file.path)");
    expect(watcher).toContain(
  ...
  it("rescans the whole vault at most once per batch", () => {
    ...
    expect(
      watcher.match(/await refreshMissingExternalFiles\(rootRaw, isCurrent\);/g)
    ).toHaveLength(1);
  ...
  it("rejects stale watcher work after a vault switch", () => {
    expect(watcher).toContain("watcherRoot: string");
    expect(watcher).toContain("watcherGeneration: number");
    expect(watcher).toContain("get().vaultPath === watcherRoot");
    expect(watcher).toContain("vaultOpenGeneration === watcherGeneration");
    expect(watcher).toContain("await refreshMissingExternalFiles(rootRaw, isCurrent);");
    expect(watcher).toContain("await registerExternalFile(rel, isCurrent);");
    expect(watcher).toContain("if (!isCurrent()) return;");
