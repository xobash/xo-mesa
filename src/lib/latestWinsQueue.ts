...
export class LatestWinsQueue<K, V> {
  ...
  constructor(
    ...
    private readonly settleSupersededQueued = false
  ) {}
  ...
  prioritize(key: K): void {
    for (let i = this.queued.length - 1; i >= 0; i--) {
      if (this.queued[i].key === key) {
        ...
        if (this.settleSupersededQueued) this.rejectSupersededQueued();
        return;
      }
    }
  ...
  private rejectSupersededQueued(): void {
    for (let i = this.queued.length - 1; i >= 0; i--) {
      if (this.queued[i].id !== this.latestId) {
        const [stale] = this.queued.splice(i, 1);
        stale.reject(new SupersededTaskError());
      }
    }
  }
