...
describe("Windows UI-thread cost contract", () => {
  it("ships a restrictive production content-security policy", () => {
    const conf = JSON.parse(tauriConf) as { app: { security: { csp: string | null } } };
    expect(conf.app.security.csp).toContain("default-src 'self'");
    expect(conf.app.security.csp).toContain("object-src 'none'");
    expect(conf.app.security.csp).toContain("frame-ancestors 'none'");
  });
