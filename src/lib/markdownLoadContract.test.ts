...
//      chunk off the first-paint critical path. The import is demand-driven,
//      not module-scope, so importing Preview for the empty workspace does not
//      fetch the renderer.
//
...
describe("markdown renderer lazy-load contract", () => {
  ...
  it("MarkdownView starts the renderer import from the demand loader", () => {
    expect(markdownView).toContain("function loadRenderer()");
    expect(markdownView).toContain("void loadRenderer().then");
    expect(markdownView).not.toMatch(
      /const\s+rendererReady:[\s\S]{0,80}import\("\.\.\/lib\/markdown"\)/
    );
  });
