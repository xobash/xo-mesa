...
vi.mock("./vault", async () => {
  ...
  return {
    ...
    readNote: async (file: VaultFile) => {
      const content = memfs.get(file.path);
...
describe("applyChangeSet", () => {
  ...
  it("preserves an external update that arrives during rollback", async () => {
    memfs.set(absFor("a.md"), "# A\noriginal");
    memfs.set(absFor("b.md"), "# B\noriginal");
    const files = [md("a.md"), md("b.md")];
    const notes = { "a.md": note("a.md"), "b.md": note("b.md") };
    const ops: ProposedOp[] = [
      { kind: "create", relPath: "Research/r.md", title: "R", content: "# R" },
      { kind: "update", relPath: "a.md", title: "A", content: "# A\nMesa update", expectedBytes: "# A\noriginal" },
      { kind: "update", relPath: "b.md", title: "B", content: "# B\nMesa update", expectedBytes: "# B\noriginal" },
    ];
    ...
    const origSet = memfs.set.bind(memfs);
    memfs.set = ((key: string, value: string) => {
      if (key.endsWith("/b.md") && value === "# B\nMesa update") {
        origSet(absFor("a.md"), "# A\nexternal update");
