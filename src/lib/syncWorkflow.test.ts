...
describe('sync workflow boundaries', () => {
  ...
  it('marks a manual retry distinctly after failed files', async () => {
    ...
    await work;
    expect(deps.transfer).toHaveBeenLastCalledWith('/vault', 'one', 'test-key', null, ['a.md'], expect.any(Function));
  });
  it('does not send a failed-file retry filter for scan-only failures', async () => {
    ...
    await actions.syncNow('one');
    await actions.syncNow('one');
    expect(deps.transfer).toHaveBeenLastCalledWith('/vault', 'one', 'test-key', null, null, expect.any(Function));
  });
