...
describe("renameNote type preservation", () => {
  ...
  it("repairs attachment references, embeds, and relative Markdown paths", async () => {
    useAppStore.setState({
      contentCache: {
        ...useAppStore.getState().contentCache,
        "refs.md": [
          "[[docs/Scan.pdf]] ![[docs/Scan.pdf|scan]]",
          "[open](docs/Scan.pdf#page=2) ![preview](docs/Scan.pdf)",
          "`![[docs/Scan.pdf]] and [open](docs/Scan.pdf)`",
          "```md",
          "![[docs/Scan.pdf]] and [open](docs/Scan.pdf)",
          "```",
        ].join("\n"),
        "folder/refs.md": "![[../docs/Scan.pdf]] and [open](../docs/Scan.pdf).",
      },
    });
    ...
    expect(renamed.writes).toEqual([
      {
        ...
        content: [
          "[[docs/Contract v2.pdf]] ![[docs/Contract v2.pdf|scan]]",
          "[open](docs/Contract%20v2.pdf#page=2) ![preview](docs/Contract%20v2.pdf)",
          "`![[docs/Scan.pdf]] and [open](docs/Scan.pdf)`",
          "```md",
          "![[docs/Scan.pdf]] and [open](docs/Scan.pdf)",
          "```",
      ...
      {
        rel: "folder/refs.md",
        expected: "![[../docs/Scan.pdf]] and [open](../docs/Scan.pdf).",
        content: "![[../docs/Contract v2.pdf]] and [open](../docs/Contract%20v2.pdf).",
      },
    ]);
  });

  it("repairs reference-style Markdown definitions without changing their uses", async () => {
    useAppStore.setState({
      contentCache: {
        ...useAppStore.getState().contentCache,
        "refs.md": [
          "[download][scan] and ![preview][scan]",
          "[scan]: <docs/Scan.pdf#page=4> \"Original scan\"",
        ].join("\n"),
      },
    });

    await useAppStore.getState().renameNote("docs/Scan.pdf", "Contract v2");
    expect(renamed.writes.find((write) => write.rel === "refs.md")?.content).toBe([
      "[download][scan] and ![preview][scan]",
      "[scan]: <docs/Contract%20v2.pdf#page=4> \"Original scan\"",
    ].join("\n"));
  });

  it("does not guess which duplicate attachment a bare filename means", async () => {
    useAppStore.setState({
      files: [...useAppStore.getState().files, file("archive/Scan.pdf")],
      contentCache: {
        ...useAppStore.getState().contentCache,
        "refs.md": "Bare ![[Scan.pdf]] and [open](Scan.pdf) stay; ![[docs/Scan.pdf]] moves.",
      },
    });

    await useAppStore.getState().renameNote("docs/Scan.pdf", "Contract v2");

    expect(renamed.writes.find((write) => write.rel === "refs.md")?.content)
      .toBe("Bare ![[Scan.pdf]] and [open](Scan.pdf) stay; ![[docs/Contract v2.pdf]] moves.");
  });

  it("selects only indexed candidates in a 3,000-note attachment rename", async () => {
    const unrelatedFiles = Array.from({ length: 3000 }, (_, index) => file(`archive/note-${index}.md`));
    const unrelatedNotes = Object.fromEntries(unrelatedFiles.map((entry) => [entry.relPath, {
    ...
    const target = file("archive/linked.md");
    useAppStore.setState({
      files: [...useAppStore.getState().files, ...unrelatedFiles, target],
      notes: {
    ...
    expect(renamed.writes.map((write) => write.rel)).toEqual([target.relPath]);
    expect(renamed.writes[0].content).toBe("![[../docs/Contract v2.pdf]]");
  });
  ...
  it("repairs note embeds while preserving attachment embeds and literal Markdown examples", async () => {
    useAppStore.setState({
      contentCache: {
        ...useAppStore.getState().contentCache,
        "refs.md": [
    ...
    expect(update?.content).toBe([
      ...
      "`inline [[a]] and [details](a.md)`",
      "![[alpha]] and ![image](a.png) stay embeds.",
      "```md",
      "[[a]] and [details](a.md)",
      "```",
    ].join("\n"));
  ...
  it("does not rewrite a three-backtick example inside a four-backtick fence", async () => {
    useAppStore.setState({
      contentCache: {
        ...useAppStore.getState().contentCache,
        "refs.md": [
