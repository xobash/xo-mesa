...
export interface DeepResearchEvent {
  kind: "progress" | "finish" | "blocked" | "agent-start" | "agent-end";
  runId: string;
  requestId?: string;
  phase?: string;
  message?: string;
...
export interface ResearchFinishReply {
  status: "accepted" | "rejected" | "stopped";
  message: string;
}

/** Correlate Mesa's validation decision with the exact waiting finish call. */
export async function respondResearchFinish(
  runId: string, requestId: string, reply: ResearchFinishReply
): Promise<void> {
  await invoke("deep_research_respond", { runId, requestId, ...reply });
}
...
export async function applyChangeSet(input: {
  ...
  const doneUpdates: { relPath: string; originalContent: string; appliedContent: string }[] = [];
  ...
  const rollback = async (): Promise<string[]> => {
    ...
    for (let i = doneUpdates.length - 1; i >= 0; i--) {
      ...
      await writeNote(file, u.originalContent, u.appliedContent).catch((e) => {
        failures.push(`restore ${u.relPath}: ${String(e)}`);
      });
    }
    // Remove created files, newest first.
    for (let i = doneCreates.length - 1; i >= 0; i--) {
      const relPath = doneCreates[i];
      const file = findFile(root, relPath);
      await readNote(file)
        .then((current) => {
          if (current !== plan.steps.find((step) => step.kind === "create" && step.relPath === relPath)?.content) {
            throw new Error("created bytes changed outside Mesa");
          }
          return removeVaultEntry(root, relPath);
        })
        .catch((e) => {
          failures.push(`remove ${relPath}: ${String(e)}`);
        });
    }
    return failures;
  };
