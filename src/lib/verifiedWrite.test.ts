...
describe("persistVerifiedBytes", () => {
  ...
  it("keeps the backup itself when even the rescue copy cannot be written", async () => {
    ...
    const fs: VerifiedWriteFs = {
      ...
      async rename(oldPath, newPath) {
        ...
        if (!data) throw new Error(`Missing file: ${oldPath}`);
        if (newPath === "/vault/test.bin") {
          // The atomic commit completes with bytes that fail final verification.
          files.set(newPath, new Uint8Array());
          files.delete(oldPath);
          return;
        }
  ...
  it("promotes the original to rescue before an in-place fallback write", async () => {
    const original = new Uint8Array([1, 2, 3]);
    const next = new Uint8Array([4, 5, 6]);
    const { fs, files } = makeFs(original);
    ...
    const renameFs: VerifiedWriteFs = {
      ...
      async rename(oldPath, newPath) {
        ...
        if (newPath === "/vault/test.bin") throw new Error("EXDEV");
        const data = files.get(oldPath);
        if (!data) throw new Error(`Missing file: ${oldPath}`);
        files.set(newPath, data);
        files.delete(oldPath);
    ...
    const rescue = operations.findIndex(
      (entry) => entry.includes(".mesa-backup-") && entry.includes(".mesa-rescue-")
    );
    const targetWrite = operations.findIndex(
      (entry) => entry === "write /vault/test.bin"
    );
    expect(rescue).toBeGreaterThanOrEqual(0);
    expect(targetWrite).toBeGreaterThan(rescue);
    expect(files.get("/vault/test.bin")).toEqual(next);
    expect([...files.keys()]).toEqual(["/vault/test.bin"]);
  });

  it("refuses the in-place fallback when the original cannot become a rescue", async () => {
    const original = new Uint8Array([1, 2, 3]);
    const next = new Uint8Array([4, 5, 6]);
    const { fs, files } = makeFs(original);
    let targetWrites = 0;
    const fullFs: VerifiedWriteFs = {
      ...fs,
      async writeFile(path, data) {
        if (path.includes(".mesa-rescue-")) throw new Error("ENOSPC");
        if (path === "/vault/test.bin") targetWrites++;
        await fs.writeFile(path, data);
      },
      async rename() {
        throw new Error("EXDEV");
    ...
    await expect(
      persistVerifiedBytes("/vault/test.bin", next, fullFs, { kind: "file" })
    ).rejects.toThrow(/before the non-atomic write fallback/i);
