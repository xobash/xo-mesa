...
it("does not report a plateau when any heap sample is missing", () => {
  clearResourceSamples();
  for (let i = 0; i < 6; i++) recordResourceSample({ nowMs: i * 1000, working: working(), governor: governor(), activeJobs: 0, pdfRasterBytes: 0, heapBytes: i === 2 ? null : 100 });
  expect(summarizeResourcePlateau()).toMatchObject({ trend: "incomplete", heapDeltaBytes: null });
});
