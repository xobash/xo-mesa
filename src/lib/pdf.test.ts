...
describe("pdf editing core", () => {
  ...
  it("compares aligned and unaligned byte views exactly", () => {
    const source = Uint8Array.from({ length: 257 }, (_, i) => (i * 73 + 19) & 255);
    for (let aOffset = 0; aOffset < 4; aOffset++) {
      for (let bOffset = 0; bOffset < 4; bOffset++) {
        const aBuffer = new Uint8Array(source.length + aOffset);
        const bBuffer = new Uint8Array(source.length + bOffset);
        aBuffer.set(source, aOffset);
        bBuffer.set(source, bOffset);
        const a = aBuffer.subarray(aOffset);
        const b = bBuffer.subarray(bOffset);
        expect(pdfBytesEqual(a, b)).toBe(true);
        for (const index of [0, 1, 127, 255, 256]) {
          b[index] ^= 1;
          expect(pdfBytesEqual(a, b)).toBe(false);
          b[index] ^= 1;
        }
      }
    }
    expect(pdfBytesEqual(source, source.subarray(0, -1))).toBe(false);
    expect(pdfBytesEqual(source, null)).toBe(false);
  });
  ...
  it("tags its own rejection so callers never blame the pdf.js worker", () => {
    // The viewer and the thumbnail path both keep a worker warm across
    // documents and destroy it when a document wedges it. A file Mesa refused
    // before pdf.js was handed anything cannot have wedged it, and 54% of a
    // real vault's `.pdf` files are exactly that. The tag rides on the error
    // rather than on the call site, so neither caller has to remember the rule.
    let thrown: unknown;
    try {
      sanitizePdfBytes(new TextEncoder().encode("\n<!DOCTYPE html>\n<html></html>"));
    } catch (err) {
      thrown = err;
    }
    expect(isNotAPdf(thrown)).toBe(true);

    // Anything else — a pdf.js parse failure, an out-of-memory slice — is
    // untagged, so the caller falls through to discarding the worker. Guessing
    // wrong in that direction is safe; guessing wrong the other way is not.
    expect(isNotAPdf(new Error("corrupt document"))).toBe(false);
    expect(isNotAPdf(undefined)).toBe(false);
    expect(isNotAPdf("not an object")).toBe(false);
    expect(isNotAPdf(null)).toBe(false);
  });

  it("survives a frozen error object rather than throwing while reporting", () => {
    // pdf.js and the structured-clone boundary can both hand back objects that
    // refuse new properties. Tagging is a best-effort annotation; it must never
    // become a second failure on top of the one being reported.
    const frozen = Object.freeze(new Error("sealed"));
    expect(() => markNotAPdf(frozen)).not.toThrow();
    expect(isNotAPdf(frozen)).toBe(false);
  });
