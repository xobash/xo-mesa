...
describe("fast path parity with toLowerCase()", () => {
  ...
  it.each([
    ["crosses a boundary", "xxBUDGETyy", "budget", 5],
    ["does not double-count an overlap", "aaaaa", "aaa", 2],
    ["counts repeated boundary matches", "xxabababyy", "ab", 3],
    ["handles a term wider than a chunk", "xxabcdefghyy", "abcdefgh", 3],
    ["keeps a match at the final character", "xxxxk", "k", 2],
  ])("incremental scan matches one pass: %s", (_name, raw, term, chunk) => {
    const expected = scanRaw(raw, buildRawMatcher(term)!, term.length);
    const scan = createIncrementalRawScan(
      raw,
      buildRawMatcher(term)!,
      term.length,
      chunk
    );
    let steps = 0;
    while (!scan.advance()) steps++;
    expect(steps).toBeGreaterThanOrEqual(0);
    expect(scan.result()).toEqual(expected);
  });

  it("keeps incremental parity across randomized boundaries", () => {
    let seed = 0x51ced123;
    const rand = () => {
      seed ^= seed << 13;
      seed ^= seed >>> 17;
    ...
    for (let trial = 0; trial < 500; trial++) {
      ...
      const term = terms[Math.floor(rand() * terms.length)].toLowerCase();
      const expected = scanRaw(raw, buildRawMatcher(term)!, term.length);
      const scan = createIncrementalRawScan(
        raw,
        buildRawMatcher(term)!,
      ...
      while (!scan.advance()) {
        /* drain every deterministic chunk */
      }
      expect(scan.result()).toEqual(expected);
    }
  });
...
describe("canScanRawFor: compatible keyed eligibility", () => {
  beforeEach(() => resetSearchEligibility());

  it("returns exactly what canScanRaw returns on every call", () => {
    const plain = "ordinary text";
    const dotted = `x${LENGTH_CHANGING_CHAR}y`;
    // Repeated calls must not change the answer.
  ...
  it("re-derives when the same key's text changes", () => {
    // A reused path must always answer for its current text.
