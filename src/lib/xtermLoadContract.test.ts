...
describe("xterm lazy-load contract", () => {
  ...
  it("does not bind static xterm.css to the terminal JavaScript vendor chunk", () => {
    const cssGuard = viteConfig.indexOf('id.endsWith(".css")');
    const terminalVendor = viteConfig.indexOf('id.includes("/@xterm/")');
    expect(cssGuard).toBeGreaterThan(-1);
    expect(terminalVendor).toBeGreaterThan(cssGuard);
  });
