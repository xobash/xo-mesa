    ...
    importDroppedPaths: async () => {
      ...
      return {
        ...
        failures: fsCalls.partialImportFailure
          ...
          : [],
        cancelled: fsCalls.cancelledImport,
      };
    },
    readNote: async () => "",
...
describe("a failed vault mutation is reported, not swallowed", () => {
  ...
  it("reports the durable partial result when the user cancels an import", async () => {
    fsCalls.cancelledImport = true;
    await useAppStore.getState().importDropped(["/tmp/dropped.md", "/tmp/later.zip"]);
    const s = useAppStore.getState();

    expect(s.files.some((file) => file.relPath === "dropped.md")).toBe(true);
    expect(s.status).toContain("Import cancelled after 1 file");
    expect(s.status).not.toContain("Import failed");
  });
  ...
  it("returns structured per-source failures from the native import layer", () => {
    ...
    expect(body).toContain("return { created, failures, cancelled: true }");
    expect(body).toContain("return { created, failures, cancelled: false }");
    expect(storeSource).toContain("if (!cancelled && paths.length === 1 && /\\.zip$/i.test(paths[0]))");
  });
});
