      ...
      <div className="welcome-card">
        ...
        {unavailableVault && (
          <div className="welcome-unavailable" role="alert">
            <strong>
              {approvalNeeded
                ? `${unavailableVault.name} needs folder approval`
                : `${unavailableVault.name} is not connected`}
            </strong>
            <span>
              {approvalNeeded
                ? "Select the folder again once so Mesa can restore its native file access."
                : "Mesa kept this vault selected and did not open it as an empty workspace. Reconnect the drive; Mesa retries automatically while this window is open."}
            </span>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (unavailableVault.reason.includes("vault folder has not been selected in Mesa")) return;
  ...
  useEffect(() => {
    ...
    const onShiftTabKey = (e: KeyboardEvent) => {
      ...
      // Shift+Tab remains ordinary backward focus navigation inside dialogs
      // and text controls. The overlay gesture is only owned by the app shell
      // when focus is in a workspace region.
      const active = document.activeElement as HTMLElement | null;
      if (isTextEntryTarget(active) || active?.closest('[role="dialog"], dialog')) return;
    ...
    return (
      <Welcome
        ...
        onRetry={() =>
          unavailableVault &&
          void openVault(
            unavailableVault.reason.includes("vault folder has not been selected in Mesa")
              ? undefined
