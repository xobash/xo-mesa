...
async fn handle_hyper(
    ...
    if path == "/sync/file" {
        ...
        if method == HyperMethod::GET {
            let path = full.clone();
            let file = match tokio::task::spawn_blocking(move || open_sync_file_no_follow(&path))
                .await
            {
        ...
        if method == HyperMethod::PUT {
            ...
            let commit_result = tokio::task::spawn_blocking(move || staged.commit(&full)).await;
        ...
        runtime.block_on(async move {
            ...
            while running_for_thread.load(Ordering::Relaxed) {
                ...
                let slot = match connection_slots.clone().try_acquire_owned() {
                    Ok(slot) => slot,
                    Err(_) => {
                        emit_log(&app_for_thread, "warn", "[serve] connection limit reached; refused peer");
                        continue;
                    }
                };
                let root = root.clone();
...
struct ManifestBody {
    ...
    #[serde(default, rename = "journalVersion")]
...
fn legacy_journal_status(status: u16, required: bool) -> bool {
    !required && matches!(status, 404 | 405 | 501)
}
...
async fn fetch_journal_with(
    ...
    token: &str,
    required: bool,
) -> Result<Option<sync_core::SyncJournal>, String> {
    ...
    if legacy_journal_status(response.status().as_u16(), required) {
        return Ok(None);
    }
    ...
    read_journal_response(response).await.map(Some)
}

/// Deliver locally generated delete/rename intent before taking the final
/// remote manifest. Current peers apply matching operations and return their
/// journal; older peers simply reject this endpoint and retain file-only
/// compatibility for the run.
...
#[tauri::command]
pub async fn sync_retire_peer(root: String, fingerprints: Vec<String>) -> Result<(), String> {
    tokio::task::spawn_blocking(move || {
        sync_core::retire_journal_peers(Path::new(&root), &fingerprints)
    })
    .await
    .map_err(|error| error.to_string())?
    .map_err(|error| error.to_string())
...
pub async fn sync_fetch_manifest(
    ...
    let files = fetch_manifest_with(&client, &base, &token).await?.files;
    Ok(ManifestResult {
...
pub async fn sync_run(
    ...
    let advertised_journal = remote_manifest.journal_version.is_some();
    if remote_manifest
        .journal_version
        .is_some_and(|version| version != sync_core::JOURNAL_VERSION)
    {
        return Err(
    ...
    let remembered_root = root.clone();
    let remembered_fingerprint = fingerprint.clone();
    let remembered_journal = tokio::task::spawn_blocking(move || {
        sync_core::load_journal(&remembered_root)
            .map(|journal| journal.peer_bindings.contains_key(&remembered_fingerprint))
    })
    .await
    .map_err(|error| error.to_string())?
    .map_err(|error| error.to_string())?;
    let journal_required = advertised_journal || remembered_journal;
    emit_log(
        &app,
        "info",
        format!(
    ...
    let mut remote_journal = fetch_journal_with(&client, &base, &token, journal_required).await?;
    if remote_journal.is_none() {
        emit_log(
            &app,
            "warn",
            "Legacy peer explicitly lacks journal support; only file copies will sync.",
        );
    }
    if let Some(journal) = &remote_journal {
        sync_core::bind_journal_peer(&root, &fingerprint, &journal.device, true)
            .map_err(|error| error.to_string())?;
    }
    ...
    // A current receiver applies a delete/rename and we then re-fetch its
    // manifest, so an initiator cannot pull the old path back in the same run.
    if remote_journal.is_some() {
        // A peer that successfully exposed a current journal is not a legacy
        // peer. Treat a rejected or malformed exchange as a protocol failure,
        // never as permission to continue with a file-only diff.
        let received_journal = deliver_journal_with(
            &client,
            &base,
            &token,
            &local_journal,
        ...
        .await
        .map_err(|error| format!("Current peer rejected journal delivery: {error}"))?;
        remote_journal = Some(received_journal);
        remote = fetch_manifest_with(&client, &base, &token).await?.files;
    } else {
        emit_log(
            &app,
            "warn",
            "peer has no journal endpoint; deletes and renames remain local for this run",
        );
    }
...
mod tests {
    ...
    #[test]
    fn only_explicit_legacy_endpoint_absence_allows_file_compatibility() {
        for status in [404, 405, 501] {
            assert!(legacy_journal_status(status, false));
            assert!(!legacy_journal_status(status, true));
        }
        for status in [200, 400, 401, 403, 408, 413, 429, 500, 502, 503] {
            assert!(!legacy_journal_status(status, false));
            assert!(!legacy_journal_status(status, true));
        }
    }
    ...
    async fn journal_fetch_errors_fail_closed_over_http() {
        ...
        for (status, required, legacy) in [
            ...
            let result =
                fetch_journal_with(&client, &format!("http://{address}"), "test-key", required)
                    .await;
            if legacy {
    ...
    async fn journal_receive_accepts_more_than_old_one_mib_limit() {
        let mut bytes =
            br#"{"version":1,"device":"peer","next_sequence":1,"operations":[],"known":[]}"#
                .to_vec();
