...
/// Maximum encoded snapshot body accepted by either reporter transport.
///
/// The reporter clips rendered text to 60 KiB and links to 80 entries. This
/// larger cap leaves room for JSON and UTF-8 while bounding the unauthenticated
/// loopback ingress before JSON parsing allocates attacker-controlled data.
pub const SNAPSHOT_BODY_CAP: usize = 512 * 1024;
const SNAPSHOT_FRAGMENT_CAP: usize = SNAPSHOT_BODY_CAP * 4;
...
pub fn parse_snapshot_body(
    ...
    if body.len() > SNAPSHOT_BODY_CAP {
        return Err("snapshot body too large".to_string());
    }
...
pub fn snapshot_payload_from_scheme_url(url: &tauri::Url) -> Option<String> {
    ...
    if fragment.len() > SNAPSHOT_FRAGMENT_CAP {
        return None;
    }
    let payload = percent_decode(fragment);
    (payload.len() <= SNAPSHOT_BODY_CAP).then_some(payload)
}
...
fn create_webview(
    ...
    let webview = window
        .add_child(
            builder,
            LogicalPosition::new(placement.x + dx, placement.y + dy),
            LogicalSize::new(placement.w, placement.h),
        )
        .map_err(|e| format!("harness webview create failed: {e}"))?;
    if !placement.visible {
        if let Err(err) = webview.hide() {
            let _ = webview.close();
            return Err(format!("harness webview initial hide failed: {err}"));
        }
    }
    Ok(())
}
...
pub fn harness_navigate(
    ...
    let placement = HarnessPlacement {
        x,
        y,
        w,
        h,
    ...
    if let Some(existing) = app.get_webview(HARNESS_LABEL) {
        if existing.window().label() == window.label() {
            ...
            if let Err(err) = visibility_result {
                ...
                return Err(format!("harness webview visibility failed: {err}"));
            }
            let wv = existing;
            match wv.navigate(parsed.clone()) {
                ...
                Err(first_err) => {
                    let _ = wv.close();
                    return create_webview(&window, &app, parsed, placement).map_err(|retry_err| {
                        format!("{first_err} (retry also failed: {retry_err})")
                    });
                }
            }
        }
    ...
    create_webview(&window, &app, parsed, placement)
}
...
mod tests {
    ...
    #[test]
    fn parse_snapshot_rejects_an_oversized_body_before_json_parse() {
        let oversized = "{".to_string() + &"x".repeat(SNAPSHOT_BODY_CAP);
        match parse_snapshot_body(&oversized, "t", 1) {
            Ok(_) => panic!("must reject"),
            Err(error) => assert_eq!(error, "snapshot body too large"),
        }
    }
    ...
    #[test]
    fn scheme_payload_rejects_an_oversized_fragment_before_decode() {
        let encoded = "a".repeat(SNAPSHOT_FRAGMENT_CAP + 1);
        let url = tauri::Url::parse(&format!("mesa-snap://snap/#{encoded}")).unwrap();
        assert!(snapshot_payload_from_scheme_url(&url).is_none());
    }
}
